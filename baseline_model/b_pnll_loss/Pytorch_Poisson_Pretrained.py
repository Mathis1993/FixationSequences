...
class TestNet(nn.Module):
    ...
    def __init__(self, gpu=False):
        ...
        self.conv1 = nn.Conv2d(256, 128, 3, stride=1, padding=1)
        ...
        self.gauss = nn.Parameter(torch.Tensor([1]), requires_grad=True)
    ...
    def forward(self, x):
        x = self.resnet(x)
        print(x.size())
        #x = torch.sum(x, dim=1).view(x.size()[0],1,x.size()[-2], x.size()[-1])
        x = self.red_ch(x)
