...
class Center_Bias2(nn.Module):
    def __init__(self, gpu=False):
        ...
        self.gpu = gpu
        self.map = gaussian_map2(torch.randn(1,1,100,100), self.gpu)

    def forward(self, x):
        #concatenate along feature channel dimension
        scaled_map = self.map*self.a
        #to fit the batch size
        scaled_map = scaled_map.repeat(x.size()[0],1,1,1) 
        x = torch.cat((x,scaled_map),1)
        return x
...
#optimizer_sgd = optim.SGD(parameters_sgd, lr=lr*10)
